...
export async function generateMetadata({
  ...
  const { page } = await fetchBlogPageData(
    ...
    1, // ✅ Internal page parameter
    9 // ✅ Internal limit parameter
  );
...
export default async function BlogPage({
  ...
  // Server-side filtering with INTERNAL pagination
  const { page, blogs, categories, pagination } = await fetchBlogPageData(
    resolvedSearchParams?.search || "",
    resolvedSearchParams?.category || "",
    1, // ✅ Internal: Always start from page 1
    9 // ✅ Internal: Always 9 items per page
  );
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto p-5 lg:py-[60px] lg:px-[35px] space-y-12">
        ...
        {/* Blog List - Shows server-filtered data with pagination */}
        <BlogListWithPagination
          initialBlogs={blogs}
          initialSearch={resolvedSearchParams?.search}
          initialCategory={resolvedSearchParams?.category}
          serverPagination={pagination} // ✅ Pass server pagination data
        />
      </div>
